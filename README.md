...
Connection Pool : 클라이언트의 요청이 있을 때 연결되는 것이 아니라 미리 연동해둠 / 작업이 완료되면 해지하는 것이 아닌 다시 Connection Pool에 반환

성능을 향상 시킬 수 있음

이러한 기능을 수행하기 위해서 필요한 jar파일을 Tomcat 7이상 부터는 lib에 파일이 존재

DataSource : Connection Pool을 관리하는 목적으로 사용하는 객체 / 객체를 통해서 Connection을 획득
기본정보를 서버의 설정파일에 저장 후 DataSource 객체로 얻어옴

DataSource를 자바단에 작성한 것이 아닌 서버의 설정파일에 작성했기 때문에 lookup() 메서드를 사용

그렇다면 DB설정 정보를 어디에 작성해야할까?

서버파일에 작성 시 이클립스의 Server 프로젝트에서 context.xml 파일을 열면 안되고, 외부 파일로  open해서 가져와 작성해야함 이유는 자바단에서는 서버를 삭제 후 재설치가 가능하기 때문에 작성해둔 파일이 사라질 수 있고, 두 파일이 동기화가 되지 않기 때문에 외부로 파일을 열어 작성

설정파일에 작성한 내용은 자동 동기화가 되지 않기 때문에 이클립스내 서버를 삭제 후 재 설정해야만 설정파일에 작성한 내용을 읽어올 수 있음

데이터베이스 연결 정보는 서버의 설정파일에 존재함
...
1. list (상세내역을 불러오지 않음)

레코드를 담을 수 있는 class
필요한 객체

executeQuery가 반환 타입이 resultSet이기 떄문에 rs = pstmt.executeQuery

2. insert (외부로부터 VO를 받아와야함)

3. update
 비밀번호는 변경할 수도 하지 않을 수도 있음
...
6. boardDetail (외부로부터 번호 받아옴)

  상세는 if문(레코드가 있다면)이고 나머지는 반복문