...
AJAX 방식으로 서버로 데이터를 전송에 대한 확인

boaedDetail.jsp에서 상단에 page디렉티브는 별도의 servlet을 생성하지 않음
...
- 요구사항 정의서, 명세서 강사님 확인예정


Day_63 : 2023-02-28

게시판별 댓글 갯수를 보여주기 위해서 db에서 가져와야함


Day_65 : 2023-03-02

- reply Controller는 jsp문서를 보여주는 forward가 아닌 server로 부터 데이터만 받아오는    Controller

- 데이터를 파라미터로 받아오지 말고 설계 시 받아올 데이터를 URL에 직접 입력하도록 설계
URL의 데이터를 추출하는 어노테이션 @PathVariable 사용 후 변수에 저장

requestBody / JSON 등 자동 바인딩을 하기 위해서는 xml로 name 값을 받아올 때 필드명과 동일해야 가능

restController일 때는 jsp문서가 없어도 API(많은 활용!)로 메서드 방식에 따라 확인 가능

따라서, restController일 때 API 테스트 완료 후 오류가 발생한다면 view단 확인

어노테이션 역할에 따른 Logic 처리 구현하는 방법에 따라 코드작성을 고민해야함

물리적인 모델링과 화면설계 동시에 진행

맡은 파트에 필요한 기능들을  어떻게 처리하면서 필요한 api 활용 후 어떤 화면이 필요할지 고민
...
어떤 로직을 기준으로 핵심적인 관점, 부가적인 관점으로 나누어서 보고 그 관점을 기준으로 각각 모듈화(공통된 로직과 기는을 하나의 단위로 묶는 것) 하는 것


Day_66 : 2023-03-03

개발 마지막 단계에서는 에러페이지를 생성해야 함(상황대비)
...
의존성(라이브러리)에 나와있는 내용은 환경설정에 다 작성되어 있어야 하기 때문에 DB작업은 주석처리할 예정(하지 않으면 에러발생)
